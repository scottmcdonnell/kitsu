    ...
    handleInput(keep) {
      ...
      if (text) {
        ...
        if (!show) {
          ...
          const { members, membersForAts, filterMatch, itemName } = this
          if (!keep) {
            // fixme: should be consistent with At.vue
            this.$emit('at', chunk)
          }
          // add support for multiple lists of members depending on the 'at' inputted
          const list = at in membersForAts ? membersForAts[at] : members
